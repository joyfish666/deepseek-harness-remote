...
- [ ] M3：M1 手机 UI 适配层（`mobile-fit/`）——**首个版本已可用**（注入式移动端适配，不改上游）；后续可继续打磨（如更多触控优化、PWA 离线壳）

## M3：M1 手机 UI 适配层（mobile-fit）

> **定位**：M1 官方原生网站在电脑上体验最佳，手机上稍显局促。M3 在**不换界面、不改上游**的前提下，通过 dsh 官方 client 插件 seam（与 `dsh-client-ui-*` 同款机制）向官方前端注入移动端 CSS 与少量交互：
>
> - 窄屏（≤820px）自动启用：**三栏折叠为单栏**、侧栏变**抽屉**（左上角 ☰ 按钮 + 遮罩）、详情列隐藏、**输入区贴底**（含 iPhone 安全区）、触控目标 ≥44px、输入框 16px（防 iOS 聚焦缩放）；
> - **电脑宽屏完全不受影响**（仅媒体查询生效）；
> - 手机访问的还是 M1 原入口 `https://<机器名>.<tailnet>.ts.net/`，**同一个界面**，无 `/m` 路径；
> - 与 M2 完全独立：M2 是自建 PWA（换一套界面），M3 是给官方界面做移动端适配（不换界面）。

安装与自定义见 [`mobile-fit/README.md`](mobile-fit/README.md)。
