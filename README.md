...
|---|---|---|---|
| 名称 | **Tailscale 隧道 + 官方 Web UI** | **自建远程网关 + 手机 App（PWA）** | **M1 手机 UI 适配层** |
| 形态 | **原生网站**：dsh 官方 Web UI 原样（零自建代码），只是经隧道送到手机 | 自建网关 + 移动端 App 界面（`remote-gateway/`） | 给 M1 官方界面注入移动端适配（`mobile-fit/` 插件） |
| 入口 | `https://<机器名>.<tailnet>.ts.net/` | `https://<机器名>.<tailnet>.ts.net/m/` | 复用 M1 入口 `/` |
| 状态 | ✅ **完全可用（唯一完成）** | 🚧 开发中 | 🚧 开发中（首个版本已可用） |
| 定位 | **日常使用的正式入口** | 试验性移动端界面 | 让 M1 在手机上更好用 |
| 手机上的界面 | 与电脑完全一致（官方原样） | 自建移动端 UI（PWA） | 官方界面 + 移动端适配 |

> ⚠️ **重要说明**：
>
> 1. **目前只有 M1 完成了**。M2、M3 都在开发中，功能尚未完全稳定，**日常使用请认准 M1（原生网站）**。
> 2. **M2/M3 不是比 M1 更好的替代品**。三者是完全独立的方案，互不替代；M1 是原生网站，M2 是自建 App 界面，M3 只是给 M1 做手机适配。
> 3. **冲突提示**：
>    - M1 与 M3 无冲突（M3 是 M1 的叠加层）；M2 与 M1/M3 入口不同（`/m/` vs `/`），可并存。
>    - **M2 的已知冲突**：启用 M2 文件浏览需要把 dsh 目录选择器从 native 换成 browse seam，会改变 **M1 电脑端**"选择工作区"的交互（改为浏览器内浏览）。若不需要 M2 文件浏览可不做该改动。
...
| `mobile-fit/` | **M3 适配层**：注入式 client 插件，让 M1 官方界面在手机上更好用（零上游改动，纯叠加） |
...
| 网关页面 401 / 连不上（M2） | 检查 `remote-gateway/.env` 的 `GATEWAY_PASSWORD` 与手机输入是否一致；`Get-ScheduledTask -TaskName dsh-gateway` 查看任务状态；`schtasks /run /tn dsh-gateway` 立即启动 |
| 网关日志（M2） | `Get-Content $HOME\.dsh\logs\gateway.log -Tail 50`；审计：`gateway-audit.log` |
| 页面显示 "Failed to load plugins"（M3） | mobile-fit 插件未生效：确认 junction 存在（`Get-Item "$HOME\.dsh\profiles\web\node_modules\mobile-fit"`）、patch 行正确，然后**重启 dsh**（`schtasks /end /tn dsh-web`） |

## 路线图

- [x] M1：Tailscale 隧道 + 官方 Web UI 远程访问——**完全可用，日常使用入口**
- [ ] M2：自建远程网关 + 手机 App（PWA）——**开发中**（独立方案，非 M1 的替代/升级）
- [ ] M3：M1 手机 UI 适配层（`mobile-fit/`）——**开发中**（首个版本已可用；后续可继续打磨触控优化、PWA 离线壳）
