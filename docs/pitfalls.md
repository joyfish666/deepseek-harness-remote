...
| P38 | `test-remote-config-proxy.mjs` fails with 403 `forbidden` (not JSON) in Test 1 (no token) — as soon as the deployment has a real token | The test spawns the proxy child with `...process.env`; when `DSH_PROXY_TOKEN` is `setx`'d for the deployment, the ambient token leaks into the child and the token gate rejects the unauthenticated test request | The test must **delete `DSH_PROXY_TOKEN` from the child env** unless the scenario passes one explicitly (2026-08-21); the same leak class applies to any test that spawns a child against a token-gated service |
