...
| P27 | Switching sessions on the phone auto-focuses the composer and raises the keyboard | Upstream InputBar's unlock effect runs `el.focus()` on mount / sessionId change (desktop habit: select a session and type); on phones the keyboard pops with no gesture involved | mobile-fit refuses **scripted (non-gesture) focus** on the composer: when `focusin` is `isTrusted=false` and the target is the composer, `preventDefault()` (Chrome cancels the focus outright, no keyboard flash) plus a `blur()` fallback (engines where focusin is not cancelable); scripted refocus within 600ms of a touch inside the composer dock (send-button keep-focus) still passes |
