...
> 状态：**M1 已落地、完全可用（日常使用入口）；M2 开发中（M1 手机 UI 适配层，介于 M1 与自建网关之间）；M3 开发中（自建网关+PWA）**。三者独立，互不替代；上游代码 `deepseek-harness-master/` 仅作参考，**不修改、不上传**。
...
### Phase 4 —— M1 手机 UI 适配层（M2，已实现首个版本）
...
> 当前编号：
> **M1 = 原生网站（完全可用）；M2 = M1 手机 UI 适配层（开发中）；M3 = 自建网关
> + PWA（开发中）**。三者独立，可并存。
