...
> 状态：**M1 已落地、完全可用（日常使用入口）；M2 基本可用（M1 手机 UI 适配层，手机日常使用，持续打磨）；M3 开发中（自建网关+PWA）**。三者独立，互不替代；上游代码 `deepseek-harness-master/` 仅作参考，**不修改、不上传**。
...
- 手机访问 M1 原入口（`/`），无新路径，与 M3 的 `/m/` 完全独立；
- 已覆盖：抽屉导航（无图标收纳栏）、会话行操作（重命名/分叉/归档）、Enter 换行、
  设置面板全屏化与滚动、统计栏/标题横滑、内测声明持久化、上游安全提示横幅等，
  行为清单见 `docs/tutorial-m2.md` 第 5 节。
...
| G1 网关骨架（M3 子项） | `remote-gateway/`：连本机 `/api`，实现会话列表 + 发消息 + 事件流订阅 | 网关在局域网内被第二台设备调用成功 |
| G2 移动端 UI（M3 子项） | 单页界面：会话列表、消息流、审批按钮、状态展示 | 手机浏览器完整操作一轮 |
| G3 安全加固（M3 子项） | Token 认证、HTTPS、限流、审计日志 | 无 Token 请求被拒；日志可追溯 |
| G4 对外部署（M3 子项） | 经隧道/VPS 暴露，公网设备可用 | 公网手机完成全流程；本机 dsh 无改动 |
| G5（可选）插件补强（M3 子项） | 按 Phase 3 补缺口能力 | 需要的事件/操作全部可达 |

**建议起点**：M1（Tailscale + 现有 GUI）今天就能用；网关系列是项目主体开发内容。

> 📌 **里程碑命名说明（2026-08-15 更新）**：早期规划表中的 M2–M5（网关骨架、
> 移动端 UI、安全加固、对外部署）属于**自建网关**项目，现在统一归为 **M3**
> （表中以 G1–G5 表示其子项）。同时新设立 **M2 = M1 手机 UI 适配层**（Phase 4，
> `mobile-fit/`）——不换界面，只为官方界面注入移动端适配，介于 M1 与自建网关之间。
> 当前编号：
> **M1 = 原生网站（完全可用）；M2 = M1 手机 UI 适配层（基本可用，手机日常使用）；
> M3 = 自建网关 + PWA（开发中）**。三者独立，可并存。
...
| 里程碑 | 内容 | 验收标准 |
|---|---|---|
| M3.1 网关骨架 | `dsh-client.js`（RPC + WS 订阅）+ REST：会话列表、发消息、事件流 | 局域网内第二台设备 curl/WS 调用成功 |
| M3.2 移动端 UI | 会话列表/详情/发送/审批/取消 | 手机浏览器完整操作一轮 |
| M3.3 安全加固 | Token、限流、审计日志 | 无 Token → 401；未信任 Host → 403；日志可追溯 |
| M3.4 部署整合 | serve 路径/端口、自启、README | 重启后手机全流程可用；dsh 零改动 |
...
> ⚠️ M3 仍在开发中，功能尚未完全稳定；M1（原生网站）是完全可用的正式入口。M3 不是 M1 的替代品，两者完全独立。
