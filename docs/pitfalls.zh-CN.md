...
| P17 | 模型页 403、插件配置页/权限/Agent 预设空白（远程访问） | dsh 把 settings/credentials 平面钉在**本机回环**，远程域名访问一律 403，多数表面静默吞错（`catch { return }`） | **属上游安全设计，默认不要绕过**；部署 remote-config 反代（`development.zh-CN.md` 第 4 节）后可解锁 |
...
| P27 | 手机端切换会话时输入框被自动聚焦、键盘弹出 | 上游 InputBar 的 unlock 效果在 mount/sessionId 变化时执行 `el.focus()`（桌面习惯：选中会话即可直接输入）；手机端无手势参与，键盘被拉起。**坑中坑**：脚本 `focus()` 派发的 focus/focusin 事件 `isTrusted` 仍为 `true`（UA 内部聚焦步骤执行，非脚本 `dispatchEvent`），**不能用 `isTrusted` 区分用户点击与脚本聚焦**（第一版修复即因此失效） | 以**指针轨迹**为准：composer 内 1s 内的 `pointerdown` = 输入意图（放行，真实点按与发送按钮 keep-focus 都在此窗口）；其他来源的 composer 聚焦一律 `preventDefault()`（Chrome 直接取消聚焦，无键盘闪烁）+ `blur()` 兜底（不可取消的引擎）。**布局坑**：上游把 `conversation.composer.dock` 渲染成 InputBar card 的 footer —— 是 textarea 分支的*兄弟*，textarea 的 `closest('[data-slot=…composer.dock]')` 永远匹配不到；发送/工具按钮也在 card 里。因此判定命中三种形状：composer textarea 本身（自匹配）、`[data-composer-card]` 祖先、或 dock 槽位（供其包裹 composer 的布局） |
...
| P35 | 开启 Token 后，手机浏览器 WS 升级全部被拒（`cookie=null`），fetch 却正常；连接控制器每 ~10s 重试、页面永不"安定" | Chromium **不随 WebSocket 握手发送 `SameSite=Lax` cookie**（WS 的 cookie 计算路径与 fetch 不同）；登录页下发的 Lax cookie 因而永远到不了反代的升级校验 | 登录 cookie 用 **`SameSite=None; Secure`**（文档记载的组合，WS 升级会携带；页面是 HTTPS 故 Secure 无碍）；APK 的 `CookieManager.setCookie` 同步该属性 |
| P36 | 升级 dsh 至 rc.8 后，`window.__ModuleLoader__` 变为队列化 facade（`{mode, pendingQueue, load, create}`），不再是直接的 `{load}` 对象 | dsh rc.8 引入两阶段启动：HTML facade 将 `load()` 调用排队，直到 `create()` 实例化模块系统（见 `packages/client/modules/src/index.ts`） | **无需操作** —— `load({id, factory})` 签名未变；facade 透明地排队注册。若编写自定义启动代码，需在 `create()` 之前而非之后调用 `load()` |
| P38 | `test-remote-config-proxy.mjs` 第 1 组（无 token）用例返回 403 `forbidden`（非 JSON）——恰在部署设置了真实 token 之后出现 | 测试用 `...process.env` 派生子进程环境；部署通过 `setx DSH_PROXY_TOKEN` 设置的 token 泄漏进子进程，token 门禁拒绝了未认证的测试请求 | 测试必须**从子进程环境里删除 `DSH_PROXY_TOKEN`**（除非该用例显式传入 token，2026-08-21 修复）；凡是对 token 门禁服务 spawn 子进程的测试，都适用同类泄漏 |
