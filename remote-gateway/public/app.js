...
const I18N = {
  zh: {
    ...
    goalCreated: '目标已设置', goalPrompt: '目标描述', deleteSession: '删除会话（归档）',
    confirmDelete: '归档后会话将从所有列表消失。数据完整保留在电脑端，但当前无法在界面中恢复（桌面端同样如此）。确定归档？',
    deleted: '已归档', renameWs: '重命名工作区', deleteWs: '删除工作区',
  ...
  en: {
    ...
    goalCreated: 'Goal set', goalPrompt: 'Goal description', deleteSession: 'Delete (archive)',
    confirmDelete: 'The session will disappear from all lists. Data stays intact on the PC, but cannot be restored from the UI (same as desktop). Archive?',
    deleted: 'Archived', renameWs: 'Rename workspace', deleteWs: 'Delete workspace',
